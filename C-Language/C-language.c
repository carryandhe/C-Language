...
//short (short int)       短整型    %d - 打印十进制的有符号数字   %u - 打印十进制的无符号数字
...
//深度剖析数据在内存中的存储
//数据类型详细介绍
//C语言有两种类型  1、内置类型（语言本身具有的类型）   2、自定义类型（构造类型）
//类型的意义：1、使用这个类型开辟内存空间的大小（大小决定了使用范围）
//2、如何看待内存空间的视角

//整形家族：
//char      ASCII码值
//      unsigned char无符号数（最高位就是最高位的有效位）（0--255）
//      signed char有符号数（最高位为符号位）（-128--127）
//short
//      unsigned short [int]
//      signed short [int]
//int
...
//整形在内存中存储
//计算机中有符号数（整形） 有三种表示方法 原反补码 无符号数 原反补码都相同
// 正数原反补码都相同
//原码：直接将数字按照正负数的形式翻译成二进制就可以了
//反码：符号位不变 其他位按位取反（负数）
//补码：反码+1（负数）
//数据存放内存中其实存放的是补码
//在计算机系统中，整数一律用补码来表示和存储。原因在于，使用补码，可以将符号位和数值域统一处理；
...
//    //100000000000000000000000000000000 只能拿32位 所以是0
//    printf("%d\n", c);
//    return 0;
//}
...
//    //0x00000014在内存中倒着存放也就是14000000
//    return 0;
//}

//设计一个小程序来判断当前机器的字节序(小端)
//指针类型的意义：
//1.指针类型决定了指针解引用操作符能访问几个字节：char* p,*p访问了1个字节；int* p,*p访问了4个字节
//2.指针类型决定了指针+1,-1.加的或者减的的是几个字节;char* p,p+1,跳过一个字符(一个字节);int* p,p+1跳过一个整形(四个字节)
//#include<stdio.h>
//int check_sys()
//{
//    int a = 1;
...
//    //11111111111111111111111111111111
//    //11111111  a
//    //11111111111111111111111111111111  a的整型提升是按照变量的数据类型的符号位来提升的得到的是补码
//    //11111111111111111111111111111110
...
//    unsigned char c = -1;
//    //11111111
//    //00000000000000000000000011111111  c的整型提升（无符号 所以是正数 正数的原反补相同）
//
...
//    //10000000  a
//    //11111111111111111111111110000000  a的整型提升（无符号 所以是正数 正数的原反补相同）
//    printf("%u\n", a);//4294967168
//    //%u - 打印十进制的无符号数字
//    return 0;
//}
...
//练习：输出什么(死循环)9 8 7 6 5 4 3 2 1 0 4294967295 4294967294
//#include<stdio.h>
//#include<windows.h>
//int main()
//{
...
//    //char a 在{-1 -2 ...-128 127 126 ... 3 2 1 0}循环     128+127=255
//    return 0;
//}

//练习：输出什么(死循环)
//#include<stdio.h>
//unsigned char i = 0;//0到255 恒小于等于255
//int main()
//{
//    for (i = 0; i <= 255; i++)
//    {
...
//    //1001.0
//    //(-1)^0 * 1.001 * 2^3
//    //0 (3+127)10000010 00100000000000000000000
//    //01000001000100000000000000000000    二进制转十进制     1091567616
//    printf("num的值为：%d\n", n);//1091567616
//    printf("*pFloat的值为：%f\n", *pFloat);//9.000000
//    return 0;
//}
...
//9.0
//1001.0
//(-1)^0 * 1.001 * 2^3(因为是二进制所以是2的几次方 小数点向左移动了3位 所以就是3次方)
//(-1)^S * M     * 2^E
//IEEE 754规定:对于32位的浮点数,最高位1位是符号位S,接着的8位是指数E,剩下的23位为有效数字M.
//对于64位的浮点数,最高位1位是符号位S,接着的11位是指数E,剩下的52位为有效数字M
//因为M可以写出1.xxxxx形式 规定默认第一位总是1所以就是1.M
//E为无符号整数 E（8位）0-255    E（11位）0-2047 但是科学计数法中E可以出现负数 所以规定存入内存时
//E的真实值必须再加上一个中间数 E(8位)中间数是127；E（11位）中间数是1023;
//2^10的E是10,所以保存32位浮点数时,必须保存成10+127=137,即10001001
//一个浮点数 先转二进制 然后按科学计数法表示 然后转补码  S E+127(8位) M小数点后（23位）
//0.5
//0.1
//(-1)^0 * 1.0 * 2^-1
//E = -1
...
//    //M=1.001
//    //E=2
//    //0 10000001(2+127=129) 01100000000000000000000 
...
//从二进制序列（内存）取出
//1、E不全为0或b不全为1
//指数E的计算值减去127（1023）,得到真实值
//0 10000001(2+127=129) 01100000000000000000000 
//(-1)^0 * 1.011 * 2^2      5.5
//01000000101100000000000000000000
//2、E全为0
//浮点数的指数E等于1-127（1-1023）即为真实值，有效数字M不再加上第一位的1，而是还原为0.xxxxxx的小数
...
//    char* arr[5];
//    char* (*pa)[5] = &arr;
//    //pa指针变量的名字，*说明pa是指针，[5]说明pa指向的数组是5个元素，char*说明pa指向的数组元素的类型是char*
//    
//    int arr2[10] = { 0 };
...
//        {
//            //printf("%d ", *(*(p + i) + j));
//            //(p+i)跳过i行 然后解引用*找到这一行 相当于找到这一行数组名 +j找到下标为j的元素的地址 在解引用*找到i行j列的元素
//            //printf("%d ", *(p[i] + j));
//            printf("%d ", p[i][j]);
//        }
//        printf("\n");
...
//int (*parr2)[10];
//parr2是一个指针,该指针指向了一个有10个元素的数组,每个元素的类型是int parr2是数组指针
